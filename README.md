...
脚本是无状态的：每个分片请求都是一次独立的运行。节点速度、退避、统计和设置都通过 Shadowrocket 的持久存储跨运行保存。节点选择规则移植自上游：热身阶段把块撒到所有节点一次测速，之后按速度排序，快的多分，再带一个没测过的节点探路；失败的节点按 3、6、12、24、48 秒退避；返回 403 这类拒绝的节点退避五分钟，而 412、429 是服务器嫌并发太高，只短暂退避该节点，同时三分钟内所有请求的线程数减半（取自上游 0.9.4.0）。一块迟迟不回来时（超过按最快节点估算时间的 1.5 倍），会再向另一个节点要一份副本，先到先用，这样一个卡住的节点拖不慢整段；输掉的副本在后台跑完，结果只用来更新节点速度。
