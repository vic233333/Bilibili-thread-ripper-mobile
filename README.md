...
- **默认就留在 App 自己那个节点上。** 它是 B 站按这台设备调度出来的，而且 App 自己的请求（音轨、太小的段、放过的段）一直在用它，连接常年是热的；我们换过去的节点每次都要重新握手，换帅后的第一段平均多花一秒半。所以「自动」模式下它是首选，只有两种情况才离开它：它被退避（连续失败、403），或者别的节点有新鲜的整段实测而且快上三倍。0.6.0 一度让领跑位落到香港节点并在那里待了四分半，每段 3 秒多，而同期 App 原本的节点只要 130 毫秒——就是因为没有这条。
...
| 单个分片总时限 | 8 秒 | 到点没拼完就原样交回，让 App 自己去拿。曾经按「App 三秒就重发」压到 3 秒，真机上直接翻车：环境自己的连接超时就要两秒，一次失败之后连重试的余地都没有，失败率从 3% 涨到 44%。预算要够试三四次 |
...
- 顺带一条：副本的等待时间原本按领跑者自己的块耗时估、封顶 2.5 秒，于是节点越慢副本开得越晚、越收不到新证据，形成自锁。封顶改成 1.5 秒。
