...
| CDN 模式 | 自动（按测速） | 自动：App 原本的节点加全部大陆、海外节点一起候选，只用测得最快的几个。另有大陆、海外、自定义三种，与浏览器版相同。哪组节点快因网络而异（见下文真机结论），不确定就先跑一次节点测速 |
...
| 单个分片总时限 | 10 秒 | 到点没拼完就交回原连接。再长也没用：App 等不到三秒就自己重发了 |
...
**2026-09-22 晚的 127 行日志，一句话结论：别混节点。** 把每个多线程分段按「用了几个节点」分组，界限干净得不像话：
