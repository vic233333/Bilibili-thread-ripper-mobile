...
- **看整段的成绩，不看单块。** 一段里某个节点拿下八成以上的块时，这一段的总吞吐才记到它头上。单块的测量（试探块、救急的副本）偏乐观——块小、连接是新的——只作参考，算分时打对折。
- **跌得快，涨得慢。** 速度记分对下降的样本给七成权重、对上升的给三成。节点崩了一两次测量就掉下来，而一次侥幸的快样本抬不动它。
- **换人要有明显优势。** 挑战者的分数要超过在位者的 1.5 倍才换。只有单块证据的挑战者因为打了对折，实际要快三倍。在位者被退避时立刻换人。
...
**0.4.0 的 169 个分段，一句话结论：选谁领跑就是全部。** 把每段按「主节点」分组：
