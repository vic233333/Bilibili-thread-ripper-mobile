...
页面下方还有：节点记忆（每个节点的状态、速度、连败次数）、放过原因统计、最近 12 个请求的明细，以及清空统计、清空节点记忆、重新检测环境、查看日志、导出诊断 JSON 的按钮。「恢复默认设置」把上表里的每一项都退回默认值，别的数据不动；「全部重置」则是设置、统计、节点记忆、环境判断、日志一起清掉。所有数据都在本机，页面不联网；明细里只显示节点和 Range，不显示带签名的完整地址。
